...
def _validate_params(request, schema):
    try:
        ...
        snacks_number = (
            int(request.args.get('snacks_number'))
            if request.args.get('snacks_number') is not None
            else 0
        )
        meals = (
            request.args.get('meals').split(',')
            if request.args.get('meals') is not None
            else ['breakfast', 'lunch', 'dinner']
        )
        time = (
            int(request.args.get('time'))
            if request.args.get('time') is not None
            else 0
        )
        price = (
            Decimal(request.args.get('price'))
            if request.args.get('price') is not None
            else 0
        )
        veggie_only = (
            bool(request.args.get('veggie_only'))
    ...
    if snacks_number < 0 or snacks_number > 2:
        errors[
            'snacks_number'
        ] = 'param \'snacks_number\' is not between 0 and 2'
    ...
    for meal in meals:
        if meal not in [
            ...
            errors[
                'meals'
            ] = 'param \'meals\' contains a value not supported for a meal'
            continue

    if (
        time < 0 and time > 450
    ):  # 450 minutes is 7.5 hours of cooking + washing
    ...
    params = dict(
        days=days,
        snacks_number=snacks_number,
        meals=meals,
        time=time,
        price=price,
        veggie_only=veggie_only,
    )
    return params
